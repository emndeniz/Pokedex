...
private class MockView : DetailsViewInterface {
    ...
    func updateUI(model: CataPoke.CompleteDetailResponse) {
        updateUICalled = true
        self.model = model
    }
...
private class MockWireframe: DetailsWireframeInterface {
    ...
    func showAlert(with title: String?, message: String?, actions: [CataPoke.Action]) {
        isShowAlertCalled = true
        self.alertMessage = message
        self.alertTitle = title
    }
    ...
    private class MockInteractor: DetailsInteractorInterface {
        ...
        func getDetails(url: URL, completion: @escaping CataPoke.DetailsInteractorCompletion) {
            if isReturnFailure {
